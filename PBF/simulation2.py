...
PARTICLE1_COLOUR = 0x328ac1
PARTICLE2_COLOUR = 0xf99613
PARTICLE3_COLOUR = 0xff0000
PARTICLE_RADIUS = 4
...
PARTICLE4_MASS = 0.7
PARTICLE5_MASS = 2
RHO_01 = PARTICLE1_MASS * (POLY6_CONST * (KERNEL_SIZE_SQR) ** 3) * 0.5
RHO_02 = PARTICLE2_MASS * (POLY6_CONST * (KERNEL_SIZE_SQR) ** 3) * 0.5
RHO_03 = PARTICLE3_MASS * (POLY6_CONST * (KERNEL_SIZE_SQR) ** 3) * 0.5
RHO_04 = PARTICLE4_MASS * (POLY6_CONST * (KERNEL_SIZE_SQR) ** 3) * 0.5
RHO_05 = PARTICLE5_MASS * (POLY6_CONST * (KERNEL_SIZE_SQR) ** 3) * 0.5
...
rho0 = ti.field(dtype=ti.f32, shape=NUM_PARTICLES)
...
def reset_particles():
    for i in range(NUM_PARTICLES_ROW):
        for j in range(NUM_PARTICLES_COL):
            index = i * NUM_PARTICLES_COL + j
            x[index][0] = 50 + j * KERNEL_SIZE + ti.random()
            x[index][1] = 50 + i * KERNEL_SIZE * 0.5 + ti.random()
            v[index] = 0, 0

            # Two types of liquids are mixed randomly
            if index == 0:
                x[index] = [150, 700]
                rho0[index] = RHO_03
                masses[index] = PARTICLE3_MASS
            elif index == 1:
                x[index] = [300, 700]
                rho0[index] = RHO_04
                masses[index] = PARTICLE4_MASS 
            elif index == 2:
                x[index] = [450, 700]
                rho0[index] = RHO_05
                masses[index] = PARTICLE5_MASS 
            elif (ti.random() < 0.5):
                rho0[index] = RHO_01
                masses[index] = PARTICLE1_MASS
            else:
                rho0[index] = RHO_02
                masses[index] = PARTICLE2_MASS
...
def solve_iter():
    ...
    for x1 in x_new:
        sum_grad_pk_C_sq = 0.
        rho_i = poly6_kernel(0) * masses[x1]
        sum_grad_pi_C = ti.Vector([0., 0.])
        for i in range(num_neighbours[x1]):
            x2 = neighbours[x1, i]
            r = x_new[x1] - x_new[x2]
            grad = spiky_grad_kernel(r) / rho0[x1]
            sum_grad_pi_C += grad
            sum_grad_pk_C_sq += grad.norm_sqr()
            rho_i += poly6_kernel(r.norm_sqr()) * masses[x2]
    ...
    for x1 in x:
        x_new[x1] = x_new[x1] + dx[x1] / rho0[x1]
...
def render(gui):
    ...
    particle_type = masses.to_numpy()
    for i in range(NUM_PARTICLES):
        if particle_type[i] == PARTICLE1_MASS:
            gui.circle(pos=q[i], color=PARTICLE1_COLOUR, radius=PARTICLE_RADIUS)
        elif particle_type[i] == PARTICLE2_MASS:
            gui.circle(pos=q[i], color=PARTICLE2_COLOUR, radius=PARTICLE_RADIUS)
        else:
            gui.circle(pos=q[i], color=PARTICLE3_COLOUR, radius=30)
    gui.show()
