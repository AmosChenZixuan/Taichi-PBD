...
def emit_smoke():
    for i in range(NUM_SMOKE_ROW):
        for j in range(NUM_SMOKE_COL):
            ...
            x[index][0] = 290 + j *KERNEL_SIZE * 0.1 + ti.random()
            x[index][1] = 20 + i * KERNEL_SIZE * 0.5 + ti.random()
            v[index] = 0, 0 
            RADIUS[index] = 3
            mass[index] = 6
            rho_0[index] = calc_rho(mass[index])
                ...
                for grid_x in ti.static(range(-1, 2)):
                    if 0 <= grid_idx[0] + grid_x < GRID_SHAPE[0]:
                        for i in range(num_particles_in_grid[grid_idx[0] + grid_x, grid_idx[1] + grid_y]):
                            ...
                            # if x1 < NUM_WATER and x2 >= NUM_WATER:
                            #     continue
                            # if x2 < NUM_WATER and x1 >= NUM_WATER:
                            #     continue
...
def solve_iter():
    ...
    for x1 in range(NUM_PARTICLES[None]):
        ...
        for i in range(num_neighbours[x1]):
            ...
            dx[x1] += (lambda_[x1] + lambda_[x2] + s_corr) * spiky_grad_kernel(r) * factor
...
def box_collision():
    ...
    for i in range(NUM_PARTICLES[None]):
        ...
        if i < NUM_WATER:
            if p[i][1] > 600 + 2*RADIUS[i]:
                p[i][1] = 600 + 2*RADIUS[i]
        else:
            if p[i][1] > 600 - RADIUS[i]:
                p[i][1] = 600 - RADIUS[i]
                #active[i] = 0
...
def update():
    for i in range(NUM_PARTICLES[None]):
        ...
        if i >= NUM_WATER:
            temp_v = ti.Vector([0.0, 0.0])
            temp_w = 0.0
            for n in range(num_neighbours[i]):
                if n < NUM_WATER:
                    j = neighbours[i, n]
                    #r = x[i] - x[j]
                    #w = poly6_kernel(r.norm_sqr())

                    temp_v += 1.0 * v[j]
                    temp_w += 1.0
                    
                    if 250 < x[j][0] < 350:
                        v[j][0] += 50*ti.random() - 50*ti.random()
                        v[j][1] += 1.0
            v[i] = temp_v / temp_w / 2
