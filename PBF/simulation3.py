...
MAX_OBSTACLE_NEIGHBOUR = 64
ov = ti.Vector.field(2, dtype=ti.f32, shape=NUM_OBSTABLES)
op = ti.Vector.field(2, dtype=ti.f32, shape=NUM_OBSTABLES)
om = ti.field(ti.f32, shape=NUM_OBSTABLES)
...
obstacle_neighbour = ti.field(ti.i32, shape=(NUM_OBSTABLES, MAX_OBSTACLE_NEIGHBOUR))
...
def reset_particles():
    for i in range(NUM_PARTICLES_ROW):
        for j in range(NUM_PARTICLES_COL):
            x[i * NUM_PARTICLES_COL + j][0] = 50 + j * KERNEL_SIZE + ti.random()
            x[i * NUM_PARTICLES_COL + j][1] = 350 + i * KERNEL_SIZE * 0.7 + ti.random()
            v[i * NUM_PARTICLES_COL + j] = 0, 0

    obstacles[0] = (100, 200)
    obstacle_radius[0] = 50
    obstacles[1] = (300, 280)
    obstacle_radius[1] = 30
    obstacles[2] = (500, 360)
...
def apply_external_forces(mouse_x: ti.f32, mouse_y: ti.f32, attract: ti.i32):
    ...
    for i in op:
        ov[i][1] = ov[i][1] + dt * -980   # gravity
        op[i] = obstacles[i] + dt * v[i]
                ...
                for grid_x in ti.static(range(-1, 2)):
                    if 0 <= grid_idx[0] + grid_x < GRID_SHAPE[0]:
                        for i in range(num_particles_in_grid[grid_idx[0] + grid_x, grid_idx[1] + grid_y]):
                            ...
                            if ((x_new[x2] - x_new[x1]).norm_sqr() < KERNEL_SIZE_SQR
...
def box_collision():
    ...
    for i in op:
        r = obstacle_radius[i]
        if op[i][0] < r:
            op[i][0] = r + COLLISION_EPSILON * ti.random()
        if op[i][0] > WIDTH - r:
            op[i][0] = WIDTH - r - COLLISION_EPSILON * ti.random()
        if op[i][1] < r:
            op[i][1] = r + COLLISION_EPSILON * ti.random()
...
def update():
    for i in range(NUM_PARTICLES):
        v[i] = (x_new[i] - x[i]) / dt * 0.99
        x[i] = x_new[i]
    
    for i in range(NUM_OBSTABLES):
        ov[i] = (op[i] - obstacles[i]) / dt * 0.1
        obstacles[i] = op[i] 
...
def render(gui):
    ...
    for i in range(NUM_OBSTABLES):
        gui.circle(pos=obstacles_display[i], color=OBSTACLE_COLOUR, radius=obstacle_radius_display[i])
    for i in range(NUM_PARTICLES):
